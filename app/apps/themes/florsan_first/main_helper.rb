  ...
  module FlorsanFirst
    module MainHelper
      def self.included(klass)
        klass.helper_method [:get_taxonomy] # put here your methods accessible from views
      rescue StandardError
        ''
      end

      def florsan_first_load_app; end
      ...
      def florsan_first_on_install_theme(theme)
        ...
        theme.add_field({ 'name' => 'Footer message', 'slug' => 'footer' },
                        { field_key: 'editor', default_value: 'Copyright 2020 - Florsan. All rights reserved.' })
      end

      # callback executed after theme uninstalled
      def florsan_first_uninstall_theme(theme); end
    end
  end
end
